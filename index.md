## Словарь DPS

Это Пали-Англо-Русский словарь с большим колличеством фунций работающих с помощью программы GoldenDict, или в любом другом приложении поддерживающим Stardict формат словаря.

Devamitta Pāḷi Study образовательный проект и находится на стадии тестирования и редактирования. Ваши предложения и комментарии будут неоценимой помощью в улучшении качества данной работы и будут полезны всем пользователям словаря.

Последнее обновление DPS можно скачать с [Google Drive](https://drive.google.com/drive/folders/18Js2o8p4DF42ViwiDO5s7VwNcolSNwhL?usp=sharing)

Здесь инструкции как установить и настроить GoldenDict на ([Windows](https://devamitta.github.io/install_win_ru.html) / [Mac](https://devamitta.github.io/install_mac_ru.html) / [Linux](https://devamitta.github.io/install_linux_ru.html) / [Android](https://devamitta.github.io/install_andoid_ru.html))

Далее можно [настроить горячие клавиши](https://devamitta.github.io/setup_hotkey_ru.html) или попробовать [всплывающее окно](https://devamitta.github.io/setup_scan_popup_ru.html). Это позволит вам в один клик открыть выделенное слово на Пали в GoldenDict.

<!-- There are some powerful [advanced settings]() for Goldendict that might be useful to you. -->

После успешной установки и настройки, здесь описание основных [особенностей](https://devamitta.github.io/features_ru.html) доступных в DPS. 

Если вы обнаружили ошибку или у вас есть идея об улучшении словаря, пожалуйста [сообщите здесь](https://docs.google.com/forms/d/1iMD9sCSWFfJAFCFYuG9HRIyrr9KFRy0nAOVApM998wM/viewform?usp=pp_url&entry.1433863141=digitalpalidictionary.github.io). Обычно это не должно занять больше минуты. 

В дополнение можно установить [больше Пали словарей](https://github.com/simsapa/simsapa-dictionary/releases) в GoldenDict. *combined-dictionary-stardict.zip* содержит их все в одном файле.

Очень ценно иметь взгляд со стороны и свежие идеи. Пожалуйста не стесняйтесь и [пишите на почту](mailto:rusdevamitta@gmail.com). Также если вы программист или профессионал в Пали или как-то хотите поддержать проект, любая [помощь](mailto:rusdevamitta@gmail.com) приветствуется!

Как только вы установили и настроили все, проверяйте наличие, [обновлений](https://devamitta.github.io/update_ru.html) DPS регулярно, чтобы иметь доступ к последней информации словаря.

Эта работа основывается на [Digital Pāli Dictionary](https://digitalpalidictionary.github.io/index.html)

Devamitta Pāḷi Study имеет лицензию [С указанием авторства-Некоммерческая-С сохранением условий версии 4.0 Международная](https://creativecommons.org/licenses/by-nc-sa/4.0/legalcode.ru).

<a rel="license" href="http://creativecommons.org/licenses/by-nc/4.0/"><img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by-nc/4.0/88x31.png" /></a><br />
